# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Black Box Movie Regex de cast247
...
def castdos(params):
    plugintools.log("[blackbox.movie-0.3.0].cast247 "+repr(params))
    url_user = {}
    ...
    if url_user.get("pageurl").find("domain=") <= 0:
        ...
        if body.find("Private Channel!") > 0 :
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Canal privado", 3 , art+'icon.png'))
            return 0
        else:
