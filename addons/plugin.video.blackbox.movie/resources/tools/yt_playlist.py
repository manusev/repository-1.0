# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Youtube Playlist para Black Box Movie
...
def yt_playlist(params):
    ...
    for entry in canal:
        ...
        plugintools.add_item(action="play", title=str(i)+'. '+vid_title+ ' [COLOR white][COLOR red][COLOR white][/COLOR]', url=vid_url, thumbnail = vid_thumb, fanart=fanart, folder=False, isPlayable=True)
        i = i + 1


# Petición de la URL
def load_more_vids(params):
    plugintools.log("blackbox.movie.gethttp_headers "+repr(params))
    ...
    for entry in matches:
        ...
        i = int(i) + 1
        print i
        plugintools.add_item(action="play", title=str(i)+'. '+vid_title+ ' [COLOR white][COLOR red][COLOR white][/COLOR]', url=vid_url, thumbnail = vid_thumb, fanart=art+'youtube.png', folder=False, isPlayable=True)
...
def header_pl(data, title_header):
    ...
    if title == "":
        ...
        try:
            title_header = '[COLOR blue][B]'+title+'[COLOR white] '+hit_list[1]+' [COLOR green]'+hit_list[2]+'[/B][/COLOR]'
            plugintools.add_item(action="", title = title_header, url = "", folder=False, isPlayable=False)
            return title_header
        except:
            title_header = '[COLOR blue][B]'+title+'[COLOR white] '+hit_list[1]+' [/B][/COLOR]'
            plugintools.add_item(action="", title = title_header, url = "", folder=False, isPlayable=False)
            return title_header            
