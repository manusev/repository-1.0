# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Black Box Movie Updater v0.2 (21.10.2014)
...
def check_update(params):
    plugintools.log("[blackbox.movie-0.3.0].check_update "+repr(params))
    ...
    for entry in subchannel:
        ...
        if title_low.find("blackbox.movie") >= 0:
            f = open(home + 'default.py', 'r')
            filename = 'plugin.video.blackbox.movie-' + last_update + '.zip'
            ...
            if actualizar == instalada :
                plugintools.log("No es necesario actualizar Black Box Movie")
            else:
                # Hay que actualizar, así que mostramos la entrada
                plugintools.log("Actualizar Black Box Movie")                
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-blackbox.movie/settings/
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-blackbox.movie/
...
def update_file(params):
    plugintools.log("[blackbox.movie-0.3.0].Update_now "+repr(params))
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Actualización fallida", 3 , art+'icon.png'))    
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Error al sobreescribir", 3 , art+'icon.png'))
    ...
    if yesno == 1:
        progreso.close()
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Actualización completada", 3 , art+'icon.png'))


def update_palco(params):
    plugintools.log("[blackbox.movie-0.3.0].update_palco "+repr(params))
    xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Espere...", 3 , art+'icon.png'))
    ...
    try:      
        ...
        pass
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box Movie', "Error en la descarga", 3 , art+'icon.png'))       
...
def parser_title(title):
    plugintools.log("[blackbox.movie-0.3.0].parser_title " + title)
