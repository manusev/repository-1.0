# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Buscador Unificado de Magnets para Black Box Movie (Kickass, Isohunt, BitSnoop, Monova)
...
def monova0_bum(params):
    ...
    try:
        ...
        texto = texto.lower()
        if texto == "":
            errormsg = plugintools.message("Black Box Movie","Por favor, introduzca el término a buscar")
            #return errormsg
        else:
            texto = texto.lower().strip()
            texto = texto.replace(" ", "+")
