...
__home__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/', ''))
...
__art__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/art', ''))
__cbx_pages__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/art/cbx', ''))
__icons__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/art/icons', ''))
__libdir__ = xbmc.translatePath(os.path.join('special://xbmc/system/players/dvdplayer/', ''))
__tools__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/resources/tools', ''))
__addons__ = xbmc.translatePath(os.path.join('special://home/addons/', ''))
__resources__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.black.box.tv/resources', ''))
...
from resources.tools.arena import *
from resources.tools.goltv import *
from resources.tools.mebuscan import *
from resources.tools.torrentfinder import *
...
from resources.tools.stream4k import *
from resources.tools.playtvpw import *
from resources.tools.wuarron import *
from resources.tools.ertete import *
#from resources.tools.documania import *
from resources.tools.elitetorrent import *
from resources.tools.torrentz import *
...
# Entry point
def run():
    plugintools.log('[%s %s] ---> black.box.tv.run <--- ' % (addonName, addonVersion))
...
def main_list(params):
    ...
    # Control del skin de Black Box TV
    mastermenu = xml_skin()
    plugintools.log("XML menu: "+mastermenu)
    try:
        data = plugintools.read(mastermenu)
    except:
        mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
        data = plugintools.read(mastermenu)
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "XML no reconocido...", 3 , __art__+'icon.png'))
...
def simpletv_items(params):
    ...
    if title == 'search.txt':
        ...
        data = file.readline()
        if data == "":
            ok = plugintools.message("Black Box TV", "Sin resultados")
            return ok
    else:
        title = params.get("title")
        ...
        if data.startswith("#EXTINF:-1") == True:
                    ...
                    if epg_source == "0":  # FórmulaTV
                        ...
                        if epg_channel != False:
                            try:
                                ...
                                print epg_channel[0]
                                title = title + " [COLOR red][B] " + epg_channel[0] + "[COLOR blue] " + epg_channel[1] + "[/B][/COLOR] "
            ...
            if data != "":
                ...
                if url == "#multilink":
                    ...
                    # wuarron:http://94.23.217.50:5001/digitv-canal2/index.m3u8
                    url = url.replace("wuarron:", "").replace("http://", "")
                    channel_id = plugintools.find_single_match(url, '/(.*?)/index.m3u8')
                    url = "http://"+url
                    print url
                    print channel_id
                    plugintools.add_item( action = "wuarron_token" , title = '[COLOR white]' + title + '[COLOR gold][I] [Wuarron][/I][/COLOR]', url = url , plot = channel_id , extra = show , show = show , thumbnail = thumbnail , fanart = fanart , folder = False , isPlayable = True )
                    data = file.readline()
                    i = i + 1
                    continue
                    ...
                    if cat != "":                        
                        ...
                        if referer:
                            ...
                            url = 'plugin://plugin.video.SportsDevil/?mode=1&amp;item=catcher%3dstreams%26url='+pageurl
                        urllib.quote_plus(url)                        
                        plugintools.add_item( action = "runPlugin" , title = '[COLOR white]' + title + '[COLOR red] [/COLOR]', plot = plot , url = url , thumbnail = thumbnail , show = show, fanart = fanart, folder = False, isPlayable = False)
                        plugintools.log("URL SportsDevil= "+url)
                        data = file.readline()
                        i = i + 1
                        continue                    
                ...
                if data.startswith("plugin") == True:
                    ...
                    if cat != "":
                        ...
                        if busqueda == 'search.txt':
                            ...
                            title = title.replace("#EXTINF:-1,", "")
                            url = 'plugin://plugin.video.p2p-streams/?url=' + data + '&mode=2&name='
                            plugintools.add_item( action = "play" , title = '[COLOR white]' + title + ' [COLOR blue][B][HD][/B][/COLOR]', plot = plot , url = url , thumbnail = thumbnail , fanart = fanart , show = show, folder = False , isPlayable = True )
                            data = file.readline()
                            i = i + 1
                            continue                        
...
def futbolenlatv(params):
    ...
    if dia_manana == "":  # Control para si es agenda de hoy o mañana
        plugintools.add_item(action="", title = '[COLOR blue][B]Programación Diaria Deportiva[/B][/COLOR] - [COLOR red][B]Día [COLOR green]'+ fecha + '[/B][/COLOR]', folder = False , isPlayable = False )
    else:
        dia_manana = dia_manana.split("-")
        dia_manana = dia_manana[2] + "/" + dia_manana[1] + "/" + dia_manana[0]
        plugintools.add_item(action="", title = '[COLOR blue][B]Programación Diaria Deportiva[/B][/COLOR] - [COLOR red][B]Día [COLOR green] '+ dia_manana + '[/B][/COLOR]', folder = False , isPlayable = False )
    ...
    for entry in bloque:
        ...
        if tv == "":
            ...
            plugintools.add_item(action="contextMenu", plot = plot , title = momentum + "[B][COLOR red] Horas " + '[COLOR white]' + category + '[/COLOR] ' + '[COLOR blue]' + champ + '[/COLOR]' + " " + '[COLOR green]' + rivales + '[/COLOR] [COLOR blue]' + plot + '[/B][/COLOR]' , thumbnail  = 'https://dl.dropboxusercontent.com/s/ensv9zpjdjsoie3/Horarios%20deportivos.jpg' , fanart = __art__ + 'agenda2.jpg' , folder = True, isPlayable = False)
...
def p2p_items(params):
    ...
    data = plugintools.read("http://pastebin.com/raw.php?i=hiig2fHz")
    ...
    # Controlamos el caso en que no haya thumbnail en el menú de Black Box TV
    if thumbnail == "":
...
def contextMenu(params):
    ...
    if len_canales == 1:
        ...
        if len_canales == 2:
            ...
            selector = dialog.select('Black Box TV', [tv_a, tv_b])
            ...
            selector = dialog.select('Black Box TV', [tv_a, tv_b, tv_c])
            ...
            selector = dialog.select('Black Box TV', [tv_a, tv_b, tv_c, tv_d])
            ...
            selector = dialog.select('Black Box TV', [tv_a, tv_b, tv_c, tv_d, tv_e])
            ...
            selector = dialog.select('Black Box TV', [tv_a , tv_b, tv_c, tv_d, tv_e, tv_f])
            ...
            selector = dialog.select('Black Box TV', [tv_a , tv_b, tv_c, tv_d, tv_e, tv_f, tv_g])
...
def tinyurl(params):
    ...
    xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Redireccionando enlace...", 3 , __art__+'icon.png'))
...
def xml_skin():
    ...
    # values="Black Box TV+|Latino Total|Zona Premium|Pastebin|Personalizado|Halloween"
    if xmlmaster == 'true':
        if SelectXMLmenu == '0':  # Black Box TV+
            mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
            plugintools.log("[black.box.tv.xml_skin: "+SelectXMLmenu)
            # Control para ver la intro de Reig
            ver_intro = plugintools.get_setting("ver_intro")
            if ver_intro == "false":
                xbmc.Player(xbmc.PLAYER_CORE_AUTO).play(__art__ + 'Reig.mp4')

        elif SelectXMLmenu == '1':   # Latino Total
            mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
            if mastermenu == "":
                plugintools.log("[black.box.tv.xml_skin: No definido")
                mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
                # Control para ver la intro de Black Box TV
                ver_intro = plugintools.get_setting("ver_intro")
                if ver_intro == "false":
                    xbmc.Player(xbmc.PLAYER_CORE_AUTO).play(__art__ + 'intro.mp4')

        elif SelectXMLmenu == '2':   # Zona Premium
            mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
            if mastermenu == "":
                plugintools.log("[black.box.tv.xml_skin: No definido")
                mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'
            ...
            if id_pastebin == "":
                ...
                plugintools.log("[black.box.tv.xml_skin: "+mastermenu)

        elif SelectXMLmenu == '4':   # Personalizado
            id_pastebin = plugintools.get_setting("id_pastebin")
            if id_pastebin == "":
                mastermenu = plugintools.get_setting("mastermenu")
                if mastermenu == "":
            ...
            if mastermenu == "":
                plugintools.log("[black.box.tv.xml_skin: No definido")
                mastermenu = 'http://pastebin.com/raw.php?i=hiig2fHz'  # Cargamos WIP
                # Control para ver la intro de Black Box TV
                ver_intro = plugintools.get_setting("ver_intro")
                if ver_intro == "false":
                    xbmc.Player(xbmc.PLAYER_CORE_AUTO).play(__art__ + 'intro.mp4')
...
def cbx_reader(params):
    ...
    if url.find("mediafire") >= 0:  # ***************** MEDIAFIRE *****************
        ...
        progreso.create("Black Box TV", "Iniciando descarga de [I]Mediafire[/I] en [I][B]playlists/tmp[/B][/I]\n")
        ...
        if os.path.isfile(src_cbx) is False:
            ...
            if url.endswith("cbz") == True:
                progreso.create("Black Box TV", "Descargando archivo CBZ en [I][B]playlists/tmp[/B][/I]")
            elif url.endswith("cbr") == True:
                progreso.create("Black Box TV", "Descargando archivo CBR en [I][B]playlists/tmp[/B][/I]")
            elif url.find("copy.com") >= 0:
                if url.find("cbr") >= 0:
                    progreso.create("Black Box TV", "Descargando archivo CBR de Copy.com en [I][B]playlists/tmp[/B][/I]")
                elif url.find("cbz") >= 0:
                    progreso.create("Black Box TV", "Descargando archivo CBZ de Copy.com en [I][B]playlists/tmp[/B][/I]")
            # response = urllib2.urlopen(url)
            # body = response.read()
    ...
    #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Descompresión fallida", 3 , __art__+'icon.png'))
