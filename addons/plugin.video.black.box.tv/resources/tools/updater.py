# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Black Box TV Updater v0.2 (21.10.2014)
...
def check_update(params):
    plugintools.log("[black.box.tv-0.3.0].check_update "+repr(params))
    ...
    for entry in subchannel:
        ...
        if title_low.find("black.box.tv") >= 0:
            f = open(home + 'default.py', 'r')
            filename = 'plugin.video.black.box.tv-' + last_update + '.zip'
            ...
            if actualizar == instalada :
                plugintools.log("No es necesario actualizar black.box.tv")
            else:
                # Hay que actualizar, así que mostramos la entrada
                plugintools.log("Actualizar Black Box TV")                
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-black.box.tv/settings/
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-black.box.tv/
...
def update_file(params):
    plugintools.log("[black.box.tv-0.3.0].Update_now "+repr(params))
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Actualización fallida", 3 , art+'icon.png'))    
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Error al sobreescribir", 3 , art+'icon.png'))
    ...
    if yesno == 1:
        progreso.close()
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('black.box.tv', "Actualización completada", 3 , art+'icon.png'))


def update_palco(params):
    plugintools.log("[black.box.tv-0.3.0].update_palco "+repr(params))
    xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Espere...", 3 , art+'icon.png'))
    ...
    try:      
        ...
        pass
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Black Box TV', "Error en la descarga", 3 , art+'icon.png'))       
