                    ...
                    if cat != "":  # Controlamos el caso de subcategoría de canales
                        if busqueda == 'search.txt':
                            if url.startswith("serie") == True:
                                ...
                                # plugin://PLD-Plexus/?url=http://something.torrent&mode=1&name=acestream+title   
                                title_fixed = title.replace(" ", "+").strip()
                                url = 'plugin://PLD-Plexus/?url='+url+'&mode=1&name='+title_fixed                                
                ...
                if data.startswith("plugin") == True:
                    ...
                    if cat != "":
                        if busqueda == 'search.txt':
                            ...
                            # plugin://PLD-Plexus/?url=sop://124.232.150.188:3912/11265&mode=2&name=Titulo+canal+Sopcast
                            url = 'plugin://PLD-Plexus/?url=' + data + '&mode=2&name='
                            url = url.strip()
                    ...
                    if cat != "":
                        if busqueda == 'search.txt':
                            # plugin://PLD-Plexus/?url=a55f96dd386b7722380802b6afffc97ff98903ac&mode=1&name=Sky+Sports+title
                            title = parser_title(title)
                            title = title.strip()
                            ...
                            url = url.strip()
                            print 'url',url
                            url = 'plugin://PLD-Plexus/?url=' + url + '&mode=1&name='
                            plugintools.add_item(action="play" , title = '[COLOR red][I]' + cat + ' / [/I][/COLOR][COLOR white] ' + title + ' [COLOR lightblue][Acestream][/COLOR]' , plot = plot , url = url, thumbnail = thumbnail , fanart = fanart , show = show, folder = False , isPlayable = True)
                            data = file.readline()
                            data = data.strip()
                            i = i + 1
                ...
                if data != "":
                    ...
                    if minus.find(texto) >= 0:
                        ...
                        if data.startswith("sop") == True:
                            # plugin://PLD-Plexus/?url=sop://124.232.150.188:3912/11265&mode=2&name=Titulo+canal+Sopcast
                            title_fixed = title.replace(" " , "+")
                            url = 'plugin://PLD-Plexus/?url=' + data + '&mode=2&name=' + title_fixed
                            url = url.strip()
                            results = open(__temp__ + 'search.txt', "a")
                            ...
                            title = title_fixed[0]
                            title_fixed = title.replace(" " , "+")
                            url = 'plugin://PLD-Plexus/?url=' + data + '&mode=1&name=' + title_fixed
                            results = open(__temp__ + 'search.txt', "a")
                            results.write("#EXTINF:-1," + title + '"' + filename + '\n')  # Hay que cambiar esto! No puede agregar #EXTINF:-1, si no es una lista m3u
                            results.write(url + '\n\n')
                            results.close()
            ...
            while data <> "" :
                if data.startswith("#") == True:
                    ...
                    if url.startswith("yt_channel") == True:
                        ...
                        # plugin://PLD-Plexus/?url=sop://124.232.150.188:3912/11265&mode=2&name=Titulo+canal+Sopcast
                        title = parser_title(title)
                        url = 'plugin://PLD-Plexus/?url=' + url + '&mode=2&name='
                        url = url.strip()
    ...
    while i <= num_items:
        if data == "":
            ...
            if data.startswith("sop") == True:
                ...
                title = parser_title(title)
                print title
                url = 'plugin://PLD-Plexus/?url=' + data + '&mode=1&name='
...
def xml_skin():
    ...
    if xmlmaster == 'true':
        if SelectXMLmenu == '0':  # Default skin
            mastermenu = base64.b64decode('aHR0cHM6Ly9kbC5kcm9wYm94dXNlcmNvbnRlbnQuY29tL3MvZnRuMDF4OHJ4aW93dWcxL01vbnN0ZXJUVi54bWw=')
            ...
            if id_pastebin == "":
                ...
                if mastermenu == "":
                    plugintools.log("[MonsterTV.xml_skin: No definido")
                    mastermenu = base64.b64decode('aHR0cDovL2xpc3Rham1kbC5wcm95ZWN0b2x1emRpZ2l0YWwuY29tL3NraW4vUExEVmlzaW9uVFY1NGwueG1s')
